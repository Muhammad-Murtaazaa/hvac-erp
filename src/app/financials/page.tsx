...
function FinancialsPageContent() {
  ...
  const [rollbackVoucher, setRollbackVoucher] = useState<any>(null);
  ...
  const handleOpenEditModal = (entry: any, details?: any) => {
    const vNum = entry.sourceId || entry.voucherNumber || entry.referenceId;
    const narration = entry.narration || entry.description || "";
    const amountVal = entry.totalDebit || entry.totalCredit || entry.amount || details?.amount || 0;
    const dateStr = entry.entryDate ? new Date(entry.entryDate).toISOString().split("T")[0] : "";

    setEditingVoucher({
      id: entry.id,
      voucherNumber: vNum,
      sourceId: vNum,
      partyName: details?.partyName || entry.partyName,
      amount: amountVal,
    });
    setEditNarration(narration);
    setEditAmount(String(amountVal));
    setEditDate(dateStr);
    setIsEditModalOpen(true);
  };

  const handleSaveVoucherEdit = async () => {
    if (!editingVoucher?.voucherNumber) return;
    ...
    try {
      ...
      toast({
        ...
        message: `Voucher ${editingVoucher.voucherNumber} has been updated successfully.`,
        type: "success",
      });
  ...
  const handleOpenRollbackModal = (entry: any, details?: any) => {
    const vNum = entry.sourceId || entry.voucherNumber || entry.referenceId;
    const amountVal = entry.totalDebit || entry.totalCredit || entry.amount || details?.amount || 0;
  ...
  const handleConfirmRollback = async () => {
    if (!rollbackVoucher?.voucherNumber) return;
    ...
    try {
      ...
      const res = await fetch("/api/finance/vouchers", {
        ...
        body: JSON.stringify({
          voucherNumber: rollbackVoucher.voucherNumber,
          reason: rollbackReason || "Super Admin financial voucher rollback",
        }),
      });
      const data = await res.json();
      ...
      toast({
        title: "Voucher Rolled Back",
        message: `Voucher ${rollbackVoucher.voucherNumber} has been safely rolled back with zero balance leaks.`,
        type: "success",
      });
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/90 dark:border-slate-800 overflow-hidden shadow-sm space-y-6 p-6 animate-fadeIn">
              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-gradient-to-br from-slate-50 via-blue-50/20 to-indigo-50/30 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 p-6 rounded-2xl border border-blue-100/80 dark:border-slate-800">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-black px-2.5 py-0.5 rounded-full uppercase tracking-widest bg-gradient-to-r from-blue-600 to-indigo-600 text-white">
                        ...
                        return (
                          <React.Fragment key={entry.id}>
                            <tr className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
                              ...
                              {/* Actions & Expand Breakdown */}
                              <td className="p-3.5 text-center align-middle whitespace-nowrap">
                                <div className="flex items-center justify-center gap-1.5">
                                  {isSuperAdminUser && (entry.sourceType === "VOUCHER" || details.sourceLabel === "Voucher") && (
                                    <>
                                      <button
                                        type="button"
                                        onClick={() => handleOpenEditModal(entry, details)}
                                      ...
                                      <button
                                        type="button"
                                        onClick={() => handleOpenRollbackModal(entry, details)}
                      ...
                      paginatedAccountingLedger.map((entry) => (
                        <tr key={entry.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
                          ...
                          <td className="p-3.5 text-center align-top whitespace-nowrap">
                            <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-950/60 px-2 py-0.5 rounded-full block w-fit mx-auto mb-1">
                              Balanced
                            </span>
                            {isSuperAdminUser && entry.sourceType === "VOUCHER" && (
                              <div className="flex items-center justify-center gap-1 mt-1">
                                <button
                                  type="button"
                                  onClick={() => handleOpenEditModal(entry)}
                                  className="p-1 rounded-md bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 hover:bg-blue-100 transition-colors cursor-pointer"
                                  title="Edit Voucher (Super Admin)"
                                >
                                ...
                                <button
                                  type="button"
                                  onClick={() => handleOpenRollbackModal(entry)}
                                  className="p-1 rounded-md bg-rose-50 dark:bg-rose-950/60 text-rose-600 dark:text-rose-400 hover:bg-rose-100 transition-colors cursor-pointer"
                                  title="Rollback Voucher (Super Admin)"
                                >
          ...
          <div className="bg-white dark:bg-slate-900 border border-rose-200 dark:border-rose-900/50 rounded-3xl max-w-md w-full p-6 sm:p-7 space-y-5 shadow-2xl animate-scaleIn my-auto">
            ...
            <div className="p-4 rounded-2xl bg-rose-50/70 dark:bg-rose-950/30 border border-rose-200/80 dark:border-rose-800/60 text-xs space-y-2">
              <div className="flex justify-between">
                ...
                <span className="font-mono font-bold text-slate-900 dark:text-white">
                  {rollbackVoucher.voucherNumber || rollbackVoucher.sourceId}
                </span>
              </div>
              {rollbackVoucher.partyName && (
                <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-mono font-black text-rose-600 dark:text-rose-400">
                  PKR {Math.round(rollbackVoucher.amount || 0).toLocaleString()}
                </span>
              </div>
            </div>

            <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
              Rolling back this voucher will safely remove its ledger entries and double-entry general journal records in an atomic transaction with zero data loss. The trial balance and party statements will immediately reflect the reversal.
            </p>
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Mistaken entry, duplicate voucher"
                className="w-full p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs focus:ring-2 focus:ring-rose-500"
              />
            </div>
