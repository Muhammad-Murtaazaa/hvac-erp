...
import { Plus, ListFilter, ClipboardCheck, ArrowUpRight, ArrowDownRight, Layers, FileText, CheckCircle2, DollarSign, RefreshCw, Undo2, QrCode, AlertCircle, AlertTriangle, Users, Phone, MapPin, Mail, Edit2, Trash2, Eye, History, User, Building2, Check, Wrench, Receipt, BookOpen, ArrowRight, Paperclip, Upload } from "lucide-react";
...
function SalesPageContent() {
  ...
  // Delivery Order Attachments / Document Upload State
  const [isDoAttachmentsOpen, setIsDoAttachmentsOpen] = useState(false);
  const [selectedDoForAttachments, setSelectedDoForAttachments] = useState<any>(null);
  const [uploadingDoFiles, setUploadingDoFiles] = useState(false);
  ...
  const handleInvoiceSubmit = async (e: React.FormEvent) => {
    ...
    const numAmountReceived = Math.max(0, Math.min(grandTotal, Number(amountReceived) || 0));
  ...
  const handleOpenEditInvoice = (inv: any) => {
    ...
    setEditAmountReceived(String(Math.round(Number(inv.amountPaid || 0))));
    setEditPayMethod(inv.payments?.[0]?.method || "CASH");
    ...
    try {
      const res = await fetch(`/api/sales/invoice/${editingInvoice.id}`, {
        ...
        body: JSON.stringify({
          ...
          amountReceived: Number(editAmountReceived) || 0,
  ...
  const handleOpenDoAttachments = (doRec: any) => {
    setSelectedDoForAttachments(doRec);
    setIsDoAttachmentsOpen(true);
  };

  const handleUploadDoFiles = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!e.target.files || e.target.files.length === 0 || !selectedDoForAttachments) return;
    const files = Array.from(e.target.files);
    setUploadingDoFiles(true);
    ...
    const token = localStorage.getItem("token");
    try {
      const res = await fetch(`/api/sales/do/${selectedDoForAttachments.id}/attachments`, {
        method: "POST",
        headers: {
      ...
      toast({
        ...
        message: `${files.length} document/photo file(s) attached to Delivery Order ${selectedDoForAttachments.doNumber}.`,
        type: "success",
      });

      // Refresh DO list and update current modal DO
      const doRes = await fetch("/api/sales/do", { headers: { Authorization: `Bearer ${token}` } });
      if (doRes.ok) {
        const data = await doRes.json();
        const list = data.deliveryOrders || [];
        setDeliveryOrders(list);
        const updated = list.find((d: any) => d.id === selectedDoForAttachments.id);
        if (updated) {
          setSelectedDoForAttachments(updated);
        }
      }
    } catch (err: any) {
      toast({ title: "Upload Failed", message: err.message, type: "error" });
    } finally {
      setUploadingDoFiles(false);
      e.target.value = "";
    }
  };

  const handleDeleteDoAttachment = async (attachmentId: string) => {
    if (!selectedDoForAttachments) return;
    const token = localStorage.getItem("token");
    try {
      const res = await fetch(
        `/api/sales/do/${selectedDoForAttachments.id}/attachments?attachmentId=${attachmentId}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      toast({ title: "Attachment Removed", message: "Document deleted successfully.", type: "info" });
  ...
  return (
    <div className="space-y-6">
                      ...
                      <tr key={doRec.id} className="hover:bg-slate-50 dark:hover:bg-slate-950/20">
                        ...
                        <td className="p-3 text-center">
                            <div className="flex items-center justify-center gap-1.5">
                              ...
                              <button
                                type="button"
                                onClick={() => handleOpenDoAttachments(doRec)}
                                className={`p-1.5 rounded transition-all relative ${
                                  (doRec.attachments?.length || 0) > 0
                                    ? "bg-purple-100 hover:bg-purple-200 dark:bg-purple-950/60 dark:hover:bg-purple-900/80 text-purple-600 dark:text-purple-300"
                                    : "bg-purple-50 hover:bg-purple-100 dark:bg-purple-950/40 dark:hover:bg-purple-900/60 text-purple-500 dark:text-purple-400"
                                }`}
                                title={`Upload & View Documents (${doRec.attachments?.length || 0} attached)`}
                              >
                                <Paperclip className="w-4 h-4" />
                                {(doRec.attachments?.length || 0) > 0 && (
                                  <span className="absolute -top-1 -right-1 bg-purple-600 text-white text-[9px] font-black rounded-full w-4 h-4 flex items-center justify-center shadow-xs">
                                    {doRec.attachments.length}
                                  </span>
                                )}
                              </button>
                            </div>
        ...
        <div className="fixed inset-0 w-screen h-screen z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl w-full max-w-4xl shadow-2xl animate-fadeIn text-slate-800 dark:text-slate-100 overflow-y-auto max-h-[90vh]">
            ...
            <form onSubmit={handleEditInvoiceSubmit} className="space-y-4">
              ...
              {/* Payment / Amount Received Section for Edit */}
              {(() => {
                const subtotal = editInvLines.reduce((acc, l) => acc + (Number(l.quantity) || 0) * (Number(l.salesPrice) || 0), 0);
                let discountAmount = 0;
                const discVal = Number(editDiscountValue) || 0;
                ...
                const numReceived = Number(editAmountReceived) || 0;
                const balanceDue = Math.max(0, grandTotal - numReceived);
                const isFull = numReceived >= grandTotal && grandTotal > 0;
                const isPartial = numReceived > 0 && numReceived < grandTotal;
                ...
                return (
                  <div className="bg-emerald-50/50 dark:bg-emerald-950/20 p-4 rounded-2xl border border-emerald-200/80 dark:border-emerald-900/50 space-y-3">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-slate-500 dark:text-slate-400">
                            Update payment collected against this invoice. Existing ledger records will be updated in-place.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-1.5 self-start sm:self-auto">
                        <button
                          type="button"
                          onClick={() => setEditAmountReceived(String(grandTotal))}
                          className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-[11px] font-bold transition-all shadow-sm"
                        >
                          Full Payment (PKR {grandTotal.toLocaleString()})
                        </button>
                        <button
                          type="button"
                          onClick={() => setEditAmountReceived("0")}
                          className="px-2 py-1 bg-white dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700 rounded-lg text-[11px] font-semibold transition-all"
                        >
                          Unpaid (0)
                        </button>
                        ...
                        <div className="relative">
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              const num = Number(val);
                              if (num > grandTotal) {
                                setEditAmountReceived(String(grandTotal));
                              } else if (num < 0) {
                                setEditAmountReceived("0");
                              } else {
                                setEditAmountReceived(val);
                              }
                            }}
                          />
                        </div>
                    ...
                    {/* Quick Installment Collector Tool */}
                    {balanceDue > 0 && (
                      <div className="bg-white/80 dark:bg-slate-900/80 p-3 rounded-xl border border-emerald-200 dark:border-emerald-800/80 space-y-2">
                        <div className="flex items-center justify-between">
                          <label className="text-[11px] font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider flex items-center gap-1.5">
                            <span>➕ Record New Installment Payment</span>
                          </label>
                          <span className="text-[10px] text-amber-600 dark:text-amber-400 font-bold">
                            Current Due: PKR {balanceDue.toLocaleString()}
                          </span>
                        </div>
                        <div className="flex flex-col sm:flex-row gap-2 items-center">
                          <div className="relative flex-1 w-full">
                            <input
                              ...
                              max={balanceDue}
                              step="any"
                              placeholder="Enter installment amount (e.g. 20000)"
                              className="w-full px-3 py-1.5 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-xs font-mono font-bold text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-emerald-500 focus:outline-none"
                              ...
                              onChange={(e) => {
                                ...
                                const num = Number(val);
                                if (num > balanceDue) {
                                  setNewInstallmentAmount(String(balanceDue));
                                } else if (num < 0) {
                                  setNewInstallmentAmount("0");
                                } else {
                                  setNewInstallmentAmount(val);
                                }
                              }}
                            />
                          </div>
                          <div className="flex items-center gap-1.5 w-full sm:w-auto">
                            <button
                              type="button"
                              onClick={() => {
                                const addVal = Number(newInstallmentAmount) || 0;
                                if (addVal <= 0) return;
                                const current = Number(editAmountReceived) || 0;
                                const updated = Math.min(grandTotal, current + addVal);
                                setEditAmountReceived(String(updated));
                                setNewInstallmentAmount("");
                              }}
                              disabled={!newInstallmentAmount || Number(newInstallmentAmount) <= 0}
                              className="flex-1 sm:flex-none px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-40 text-white rounded-lg text-xs font-bold transition-all whitespace-nowrap"
                            >
                              + Add to Received
                            </button>
                            <button
                              ...
                              className="flex-1 sm:flex-none px-2.5 py-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-lg text-xs font-semibold transition-all whitespace-nowrap"
                            >
                              Pay Remaining (PKR {balanceDue.toLocaleString()})
                            </button>
                          </div>
                        </div>
                        <p className="text-[10px] text-slate-400">
                          Entering an installment automatically updates the cumulative Amount Received and adjusts remaining balance due.
                        </p>
                      </div>
                    )}

                    {/* Payment History Breakdown if existing payments */}
                    {editingInvoice?.payments?.length > 0 && (
                      <div className="pt-2 border-t border-emerald-100 dark:border-emerald-900/40">
                        <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider block mb-1">
                          Payment Installments on Record:
                        </span>
                        <div className="space-y-1">
                          {editingInvoice.payments.map((p: any, idx: number) => (
                            <div key={p.id || idx} className="flex justify-between items-center text-[11px] bg-white/60 dark:bg-slate-900/60 px-2.5 py-1 rounded-lg border border-slate-100 dark:border-slate-800">
                              <span className="text-slate-600 dark:text-slate-300 font-semibold">
                                Installment #{idx + 1} ({p.method || "CASH"}) - {new Date(p.paymentDate || p.createdAt || Date.now()).toLocaleDateString()}
                              </span>
                              <span className="font-mono font-bold text-emerald-600 dark:text-emerald-400">
                                PKR {Math.round(Number(p.amountPaid)).toLocaleString()}
                              </span>
                            </div>
                          ))}
                        </div>
      ...
      {/* ==================== DELIVERY ORDER DOCUMENTS & ATTACHMENTS MODAL ==================== */}
      {mounted && isDoAttachmentsOpen && selectedDoForAttachments && createPortal(
        <div className="fixed inset-0 w-screen h-screen z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl w-full max-w-2xl shadow-2xl animate-fadeIn text-slate-800 dark:text-slate-100 overflow-y-auto max-h-[90vh]">
            <div className="flex justify-between items-start mb-4 border-b border-slate-100 dark:border-slate-800 pb-3">
              <div className="flex items-center gap-3">
                <div className="p-2.5 rounded-2xl bg-purple-50 dark:bg-purple-950/60 text-purple-600 dark:text-purple-400 border border-purple-200/60 dark:border-purple-800/60 shadow-xs">
                  <Paperclip className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">
                    Delivery Order Documents & Proofs
                  </h3>
                  <div className="text-xs text-slate-500 flex flex-wrap items-center gap-2 mt-0.5">
                    <span className="font-mono font-bold text-purple-600 dark:text-purple-400">
                      {selectedDoForAttachments.doNumber}
                    </span>
                    <span>•</span>
                    <span className="font-semibold text-slate-700 dark:text-slate-300">
                      {selectedDoForAttachments.clientName}
                    </span>
                    {selectedDoForAttachments.poNumber && (
                      <>
                        <span>•</span>
                        <span className="text-[11px] text-slate-400">
                          Ref PO: {selectedDoForAttachments.poNumber}
                        </span>
                      </>
                    )}
                  </div>
              ...
              <button
                type="button"
                onClick={() => {
                  setIsDoAttachmentsOpen(false);
                  setSelectedDoForAttachments(null);
                }}
                className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 text-xl font-bold transition-all p-1"
              >
                ✕
              </button>
            </div>
            ...
            <div className="space-y-4">
              {/* Upload Zone */}
              <div className="border-2 border-dashed border-purple-200 dark:border-purple-900/60 p-5 rounded-2xl text-center space-y-2.5 hover:border-purple-500 transition-all bg-purple-50/30 dark:bg-purple-950/20">
                <div className="w-10 h-10 mx-auto rounded-full bg-purple-100 dark:bg-purple-900/40 text-purple-600 dark:text-purple-300 flex items-center justify-center">
                  <Upload className="w-5 h-5" />
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-700 dark:text-slate-300">
                    Upload Signed Delivery Challans, Dispatch Proofs, or Scanned Documents
                  </p>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
                    Supports Photos (PNG, JPG, WebP), PDFs, and Word documents (Multiple allowed)
                  </p>
                </div>
                <div>
                  <input
                    type="file"
                    id="do-file-upload-modal"
                    multiple
                    className="hidden"
                    accept="image/*,application/pdf,.doc,.docx"
                    onChange={handleUploadDoFiles}
                    disabled={uploadingDoFiles}
                  />
                  <label
                    htmlFor="do-file-upload-modal"
                    className="inline-flex items-center gap-1.5 px-4 py-2 bg-gradient-to-r from-purple-600 via-indigo-600 to-blue-600 hover:from-purple-700 hover:to-indigo-700 disabled:opacity-50 text-white text-xs font-bold rounded-xl cursor-pointer transition-all shadow-md shadow-purple-500/20"
                  >
                    <Upload className="w-3.5 h-3.5" />
                    <span>{uploadingDoFiles ? "Uploading Files..." : "Upload Photos & Documents (Multiple)"}</span>
                  </label>
                </div>
              </div>

              {/* Gallery / List of Attachments */}
              <div className="space-y-2">
                <div className="flex justify-between items-center px-1">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                    Attached Files ({selectedDoForAttachments.attachments?.length || 0})
                  </span>
                </div>

                {selectedDoForAttachments.attachments && selectedDoForAttachments.attachments.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[320px] overflow-y-auto p-1">
                    {selectedDoForAttachments.attachments.map((file: any) => {
                      const isImage = file.fileType?.startsWith("image/") || /\.(jpg|jpeg|png|webp|gif)$/i.test(file.fileName);
                      return (
                        <div
                          key={file.id}
                          className="flex items-center gap-3 p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xs hover:shadow-md transition-all group"
                        >
                          {isImage ? (
                            <a
                            ...
                            <div className="flex-shrink-0 w-12 h-12 bg-purple-50 dark:bg-purple-950/40 rounded-lg flex items-center justify-center border border-purple-100 dark:border-purple-900/60 text-purple-600 dark:text-purple-400">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <button
                              type="button"
                              onClick={() => handleDeleteDoAttachment(file.id)}
                  ...
                  <div className="text-center py-8 border border-dashed border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50/30 dark:bg-slate-950/20">
                    <Paperclip className="w-8 h-8 text-slate-300 dark:text-slate-600 mx-auto mb-2" />
                    <p className="text-xs font-semibold text-slate-500">No documents attached yet</p>
                    <p className="text-[11px] text-slate-400 mt-0.5">Use the upload button above to attach signed delivery challans or receiving proofs</p>
                  </div>
                )}
              </div>
            </div>
                              ...
                              <td className="p-3 text-center">
                                  <div className="flex items-center justify-center gap-1">
                                    ...
                                    <button
                                      ...
                                      onClick={() => {
                                        setIsCustomerDossierOpen(false);
                                        handleOpenDoAttachments(d);
                                      }}
                                      className="px-2 py-1 bg-purple-50 hover:bg-purple-100 dark:bg-purple-950/40 text-purple-600 dark:text-purple-400 rounded text-[10px] font-bold inline-flex items-center gap-1"
                                      title="Upload & View Documents"
                                    >
                                      <Paperclip className="w-3 h-3" />
                                      <span>Docs {d.attachments?.length ? `(${d.attachments.length})` : ""}</span>
                                    </button>
                                  </div>
                              </td>
                            </tr>
