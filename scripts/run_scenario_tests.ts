...
async function getPartyBalances(partyId: string, partyType: "CUSTOMER" | "VENDOR" | "EMPLOYEE") {
...
async function runAllScenarios() {
  ...
  if (matches.length > 1) {
    ...
    console.log("\n--- TEARDOWN: Cleaning up all scenario test artifacts ---");
    // 1. Delete all scenario journal lines and entries
    const scenarioJEs = await prisma.journalEntry.findMany({
      where: {
        OR: [
          { idempotencyKey: { startsWith: "SCENARIO_" } },
          { idempotencyKey: { startsWith: "IDEMPOTENCY_TEST_KEY_" } },
        ],
      },
      select: { id: true },
    });
    if (scenarioJEs.length > 0) {
      const jeIds = scenarioJEs.map((j: any) => j.id);
      await prisma.journalLine.deleteMany({ where: { journalEntryId: { in: jeIds } } });
      await prisma.journalEntry.deleteMany({ where: { id: { in: jeIds } } });
    }

    // 2. Delete test parties
    await prisma.customer.deleteMany({
      where: {
        OR: [
          { name: { contains: "Scenario 1 Test Customer" } },
          { name: { contains: "Scenario 2 Test Customer" } },
          { name: { contains: "Tariq Mahmood Test" } },
        ],
      },
    });
    ...
    await prisma.vendor.deleteMany({
      where: {
        name: { contains: "Scenario 3 Test Vendor" },
      },
    });

    await prisma.employee.deleteMany({
      where: {
        name: { contains: "Scenario 4 Test Employee" },
      },
    });
